...
public class GamePanel extends JPanel implements KeyListener, MouseListener {
	...
	public GamePanel () {
		...
		try {
			...
			flappyMiniFont  = flappyFontBase.deriveFont(Font.PLAIN, 18);
	...
	private void drawMenu (Graphics g) {
		...
		// Credits :p
		drawCentered("Creators:Gaurav,Khushi & Yuvraj", FlappyBird.WIDTH, FlappyBird.HEIGHT, 580, g);
		g.setFont(flappyMiniFont); // Change font
		drawCentered("2019120046 2019120047 2019120051", FlappyBird.WIDTH, FlappyBird.HEIGHT, 610, g);
				...
				if (p.collide(
					...
					if (gameBird.getX() >= p.getX() + p.WIDTH / 2) {
						...
						if (p.canAwardPoint) {
							...
							score++;
							if (score % 5 == 0) {
								pipes.get(pipes.size() - 1).inc_speed();
							}
	...
	public void gameOver (Graphics g) {
		...
		// Medal
		if (score >= 5) { medal = "bronze"; }
		if (score >= 10) { medal = "silver"; }
		if (score >= 15) { medal = "gold"; }
		if (score >= 20) { medal = "platinum"; }
		...
		pipes.get(pipes.size() - 1).resetSPEED();
